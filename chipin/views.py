...
def create_group(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if Group.objects.filter(name__iexact=group_name).exists():  # Case-insensitive check
                messages.error(request, f'A group with the name "{group_name}" already exists. Please choose another name.')
            else:
                group = form.save()
